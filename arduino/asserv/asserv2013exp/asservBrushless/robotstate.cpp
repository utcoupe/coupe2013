...
void initRobotState(){
	...
	robot_state.y = 0;
	robot_state.speed = 0;
	robot_state.speedMotorR = 0;
	robot_state.speedMotorL = 0;
	robot_state.a = 0;
	robot_state.lastCompute = millis();
}
...
double robot_get_speedr() {
	return robot_state.speedMotorR;
}

double robot_get_speedl() {
	return robot_state.speedMotorL;
}
...
void computeRobotState(){
	...
	/*Calcul de la vitesse de chaque moteur pour l'asserv en vitesse
	 *On assimile la vitesse des codeurs à celle des moteurs	
	 *Les vitesses sont en ticks/ms					*/
	unsigned long now = millis();
	long int dt = now - robot_state.lastCompute;
	robot_state.speedMotorR = dr/dt;	
	robot_state.speedMotorL = dl/dt;	

	/*preparation de la prochaine iteration*/
	prev_value_left_enc = value_left_enc;
	prev_value_right_enc = value_right_enc;
	robot_state.lastCompute = now;

	/* calcul du deplacement */
	long int delta_dist = (dr+dl)*50;

	/*mise a jour de l'etat du robot  */
	robot_state.speed = delta_dist/dt;
