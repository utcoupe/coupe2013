 ...
 *kd			|-			|Diminue		|Diminue	|-			|
 * _____________________|_______________________|_______________________|_______________|_______________________|
 */
...
double pwm,consigne,consigneSpeedR,consigneSpeedL,currentSpeed,currentSpeedR,currentSpeedL;
PID pid4SpeedControl(&currentSpeed,&pwm,&consigne,KP_SPEED,KI_SPEED,KD_SPEED);
PID pid4SpeedRightControl(&currentSpeedR,&pwm,&consigneSpeedR,KP_SPEED,KI_SPEED,KD_SPEED);
PID pid4SpeedLeftControl(&currentSpeedL,&pwm,&consigneSpeedL,KP_SPEED,KI_SPEED,KD_SPEED);
...
double currentEcart, output4angle;
PID pid4AngleControl(&currentEcart,&output4angle,&consigne,KP_ANGLE,KI_ANGLE,KD_ANGLE);
...
void angleControl(int* value_pwm_left, int* value_pwm_right){
	...
	*/
	...
	if(current_goal.phase == PHASE_2){
		...
		if(!ASSERV_VITESSE){
			(*value_pwm_right) = output4angle;
			(*value_pwm_left) = -output4angle;
		}
		else{
			/*Avec l'asserv en vitesse, on transmet les output Alpha et Delta à l'asservissement en vitesse, qui calcule la pwm*/
			currentSpeedR = robot_get_speedr()*(255/MAX_SPEED);//Vitesse actuelle sur [-255,255]
			consigneSpeedR = output4angle;
			pid4SpeedRightControl.Compute();
			(*value_pwm_right) = pwm;
			//chaque pwm est calculée de la même manière, successivement
			currentSpeedL = robot_get_speedl()*(255/MAX_SPEED);
			consigneSpeedL = -output4angle;
			pid4SpeedLeftControl.Compute();
			(*value_pwm_left) = pwm;
		}
...
void positionControl(int* value_pwm_left, int* value_pwm_right){
	...
	if(!initDone){
		...
		pid4AlphaControl.SetOutputLimits(-current_goal.speed,current_goal.speed); /*composante lie a la vitesse de rotation*/
		pid4AlphaControl.SetMode(AUTO);
		/*Asservissement en vitesse de chaque moteur :
		 *Input [-255,255] provenant de l'asserv en position
		 *Ouput [-255,255] pour la pwm */
		pid4SpeedLeftControl.Reset();
		pid4SpeedLeftControl.SetInputLimits(-255,255);
		pid4SpeedLeftControl.SetSampleTime(DUREE_CYCLE);
		pid4SpeedLeftControl.SetOutputLimits(-255,255);
		pid4SpeedLeftControl.SetMode(AUTO);
		pid4SpeedRightControl.Reset();
		pid4SpeedRightControl.SetInputLimits(-255,255);
		pid4SpeedRightControl.SetSampleTime(DUREE_CYCLE);
		pid4SpeedRightControl.SetOutputLimits(-255,255);
		pid4SpeedRightControl.SetMode(AUTO);
		initDone = true;
	}
	...
	if (current_goal.phase == PHASE_1 and abs(currentAlpha) > M_PI/12.0) {
		//pid4AlphaControl.SetOutputLimits(-100,100); // composante liee a la vitesse de rotation; Encore une fois commenté à cause de la rampe
		onlyTurn = true;
	}
	...
	switch(current_goal.phase)
	{
		...
		case PHASE_DECEL:
			if (fifoIsEmpty())
			{
				// Commentée, la phase decel est inutile, je la conserve pour les tests
				//Cette partie était utilisée à la coupe2012 pour compenser un sserv défaillant*/
		...
		case PHASE_CORRECTION:
			/*pid4DeltaControl.SetOutputLimits(-min(50,current_goal.speed),min(50,current_goal.speed)); // composante liee a la vitesse lineaire
			pid4AlphaControl.SetOutputLimits(-150,150); // composante liee a la vitesse de rotation
			Commenté car cela rentre dans le même esprit que la rampe de desceleration*/
			if (abs(currentDelta) < 10*ENC_MM_TO_TICKS)
			{
				current_goal.phase = PHASE_ARRET;
			}
	...
	else
	{
		...
		if (!ASSERV_VITESSE){
			...
			/*Avec l'asserv en vitesse, on transmet les output Alpha et Delta à l'asservissement en vitesse, qui calcule la pwm*/
			currentSpeedR = robot_get_speedr()*(255/MAX_SPEED);//Vitesse actuelle sur [-255,255]
			consigneSpeedR = output4Delta+output4Alpha;
			pid4SpeedRightControl.Compute();
			(*value_pwm_right) = pwm;
			//chaque pwm est calculée de la même manière, successivement
			currentSpeedL = robot_get_speedl()*(255/MAX_SPEED);
			consigneSpeedL = output4Delta-output4Alpha;
			pid4SpeedLeftControl.Compute();
			(*value_pwm_left) = pwm;
		}
		/*Evite les "vibrations" dues aux pwm trop basse sur des moteurs CC, tout en conservant un eventuel rapport entre les pwm*/
